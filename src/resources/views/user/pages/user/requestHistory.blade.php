...
<nav>
    <div class="nav nav-tabs" id="nav-tab" role="tablist">
        ...
        <a class="nav-link" id="nav-contact-tab" data-toggle="tab" href="#nav-completed" role="tab" aria-controls="nav-contact" aria-selected="false">Completed</a>
...
<div class="tab-content" id="nav-tabContent">
    <div class="tab-pane fade show active" id="nav-all" role="tabpanel" aria-labelledby="nav-profile-tab">
        ...
        @endif ">
                ...
                @if (($request->status == 'confirmed' || $request->status == 'completed') && $request->rating >0)
                <td>
                    <?php $primary_stars = $request->rating;
                    $secondary_stars = 5 - $primary_stars;
                    ?>
                    @if ($primary_stars > 0)
                    @for ($i = 0; $i < $primary_stars; $i++) <i class="fa fa-star text-warning"></i>
                        @endfor
                        @endif
                        @if ($secondary_stars > 0)
                        @for ($i = 0; $i < $secondary_stars; $i++) <i class="fa fa-star text-secondary"></i>
                            @endfor
                            @endif
                ...
                @else
                <td>
                    <span class="p-2 badge badge-danger">No ratings</span>
                </td>
                @endif
                <td>
                    @if ($request->status == "rejected" || $request->status == "pending")
                    <button disabled class="btn btn-sm btn-secondary rounded-pill">Feedback</button>
                    @else
                    <button class="btn btn-sm btn-info rounded-pill" data-toggle="modal" data-target="#feedbackModel{{$key}}">Feedback</button>
                    @endif
                </td>

            </tr>
            <div class="modal fade" id="feedbackModel{{$key}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form action="{{route('user.feedback.send',$request->id)}}" method="POST">
                            <div class="modal-body">
                                @csrf
                                @method('PATCH')
                                <div class="form-group">
                                    @if ($request->status == "completed" && $request->feedback == null)
                                    <textarea class="form-control" name="feedback" cols="30" rows="10"></textarea>
                                    @else
                                    <textarea class="form-control" name="feedback" cols="30" rows="10">{{$request->feedback}}</textarea>
                                    @endif
    ...
    <div class="tab-pane fade" id="nav-completed" role="tabpanel" aria-labelledby="nav-contact-tab">
        ...
        <table class="table table-bordered table-hover">
            ...
            @foreach ($completed_services as $key => $request)
            <tr>
                <td>{{ ++$key }}</td>
                <td>{{ $request->provider->name }}</td>
                <td>{{$request->provider->profession->name}}</td>
                ...
                @if ($request->status == "rejected" || $request->status == "pending")
                    <button disabled class="btn btn-sm btn-secondary rounded-pill">Feedback</button>
                    @else
                    <button class="btn btn-sm btn-info rounded-pill" data-toggle="modal" data-target="#completedFeedbackModel{{$key}}">Feedback</button>
                    @endif
                </td>

            </tr>
            <div class="modal fade" id="completedFeedbackModel{{$key}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="exampleModalLabel">Feedback form</h5>
